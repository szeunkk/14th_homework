...
얄리얄리 얄라셩 얄라리 얄라`,
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const totalHeight = itemHeight * circularPictures.length;
      const sectionHeight = itemHeight * originalPicturesLength;

      // 맨 위로 스크롤된 경우 중간 섹션으로 이동
      if (scrollTop < itemHeight) {
        wrapper.scrollTop = scrollTop + sectionHeight;
      }
      // 맨 아래로 스크롤된 경우 중간 섹션으로 이동
      else if (scrollTop > totalHeight - sectionHeight - itemHeight) {
        wrapper.scrollTop = scrollTop - sectionHeight;
      }
    };
