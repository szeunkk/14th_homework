...
export default function CommentWrite({isEdit, onClickEdit, el}:{isEdit: boolean, onClickEdit?: () => void , el?:IComment}){

    const {writer, password, contents, onChangeWriter, onChangePassword, onChangeContents, onClickCommentSubmit, onClickCommentEdit, onClickRate, isValid, rating} = useCommentWrite({el, onClickEdit});
    ...
    return(
        ...
        <div className={styles.CommentWrite}>
            ...
            {isEdit && <Button variant="CommentBtn" type="button" onClick={onClickEdit}>취소</Button>}
            <Button variant="CommentBtn" type="submit" disabled={isValid}>{isEdit? "수정 하기" : "댓글 등록"}</Button>
            </div>
        </div>
        </form>
    )
